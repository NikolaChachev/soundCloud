...
public class PlaylistController extends SessionManagerController {
    ...
    public ResponseDto createPlaylist(@RequestBody PlaylistDto playlistDto, HttpSession session) throws NotLoggedException {
    ...
    public ResponseDto deletePlaylist( @PathVariable("plId") long playlistId,
           HttpSession session) throws NotLoggedException, DoesNotExistException, UnauthorizedUserException {
        isUserLogged(session);
        User user = (User) session.getAttribute(LOGGED);
    ...
    public ResponseDto addSongToPlaylist(@PathVariable("plId") long playlistId,@PathVariable("sId") long songId,HttpSession session) throws NotLoggedException, UnauthorizedUserException, DoesNotExistException {
        isUserLogged(session);
        Playlist playlist = playlistRepository.getById(playlistId);
        User user = (User) session.getAttribute(LOGGED);
        if(playlist == null){
